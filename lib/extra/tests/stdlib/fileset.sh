...
if test -n "${TEST_LIB:-}"; then
  NIX_PATH=nixlib="$TEST_LIB/nix/stdlib/"
else
  NIX_PATH=nixlib="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../nix/stdlib/"; pwd)"
fi
export NIX_PATH
...
prefixExpression='
  let
    lib = import <nixlib>;
    internal = import <nixlib/fileset/internal.nix> {
      inherit lib;
    };
  in
  with lib;
...
expectFailure 'with ((import <nixlib>).extend (import <nixlib/fileset/mock-splitRoot.nix>)).fileset;
